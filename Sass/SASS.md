...
```
### Mixin
1. 當CSS設定經常性地被重複使用，甚至可以根據不同「參數」對應出相似的樣式，就可以將這段設定獨立寫成一個 mixin 方便取用。
    * 如何撰寫mixin
        * 先建立一個_mixin檔案，在檔案裡撰寫需要的mixin。
        * 使用 **@include** 來混入。
```bash=
